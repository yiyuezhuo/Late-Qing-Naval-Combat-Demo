...
| I            | [14"" Whitehead](https://en.wikipedia.org/wiki/Whitehead_torpedo)                                       | CHILE / TB / [Almirante Lynch](https://en.wikipedia.org/wiki/Chilean_torpedo_gunboat_Almirante_Lynch)                                         | 1868 | 118          | 14              |";
    ...
    static readonly GlobalString Sk5CodeHelpMessage = new()
    {
        ...
        japanese = "射撃指揮コードは、SK5データにおける射撃指揮構成要素の特定の組み合わせを表します。役割、射撃指揮コード、時代、または構成要素フィールドを変更すると、利用可能な場合は標準表から射撃指揮表が更新されます。利用できない場合、その砲兵装はカスタム表を使用しているものとしてマークされます。射撃指揮コードと構成要素の値自体は判定処理に影響しません。これらの効果は、十分統計量として機能する射撃指揮表にすでに完全に反映されているためです。",
