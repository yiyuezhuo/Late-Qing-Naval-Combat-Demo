...
To move the camera, press and hold the right mouse button while moving the mouse (known as right-click dragging). This is a common method for camera movement in GIS applications and similar games. (When using touch input, drag one finger on the map to move the camera.)
...
カメラを移動するには、右クリックを押したままマウスを動かしてください（右クリックドラッグ）。これは、GISアプリケーションや同種のゲームで一般的に用いられるカメラ操作方法です。(タッチ操作では、マップ上を1本指でドラッグしてカメラを移動できます。)
...
要移动视角，请按住鼠标右键并移动鼠标（即右键拖拽）。这是一种在 GIS 程序及类似游戏中常见的视角移动方式。(使用触屏操作时，可以在地图上单指拖动来移动视角。)
...
要移動視角，請按住滑鼠右鍵並移動滑鼠（即右鍵拖曳）。這是在 GIS 應用程式及類似遊戲中常見的視角操作方式。(使用觸控操作時，可以在地圖上單指拖動來移動視角。)
