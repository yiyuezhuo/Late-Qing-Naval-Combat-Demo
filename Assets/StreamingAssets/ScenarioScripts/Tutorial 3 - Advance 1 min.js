...
射撃を行っているグループから艦艇を選択してください（本剧本では通常、日本艦隊が先に射撃します）。赤色の線が表示され、主砲・副砲・ tertiary砲（三番砲）または速射砲による目標への射撃が可視化されます。

艦艇動態状態エディターを開き「兵装」タブに移動してください。ここで射撃プロセスの情報を確認でき、特に弾薬残数と処理秒数（Processing Seconds） が注目すべき項目です。

各砲台は異なる種類の弾薬を搭載しており、AIは射撃時に最適な弲種を自動選択します。

処理秒数は時間経過とともに増加し、射撃速度（Rate of Fire）で決定された閾値に達すると、射撃が解決され処理秒数がゼロにリセットされます。飛翔中の砲弾はモデル化されません（ただし、発射された魚雷はモデル化されます）。
...
请从射击编组中选择一艘舰艇（本剧本中通常日方舰艇先开火）。红色线条将显示该舰正在使用主炮、副炮、 tertiary炮（第三级火炮）或速射炮向目标开火。

打开舰艇动态状态编辑器并进入"武器"标签页。此处可查看射击过程信息，最值得关注的条目是弹药余量和处理秒数（Processing Seconds）。

各炮组携带不同弹种，AI会在射击时自动选择最优弹药。

处理秒数随时间推进增加，直至达到射速（Rate of Fire）决定的阈值时触发射击判定，随后归零。未模拟飞行中的炮弹（但已发射的鱼雷会持续模拟）。
...
請從射擊編組中選擇一艘艦艇（本劇本中通常日方艦艇先開火）。紅色線條將顯示該艦正在使用主炮、副炮、 tertiary炮（第三級火炮）或速射炮向目標開火。

打開艦艇動態狀態編輯器並進入「武器」標籤頁。此處可查看射擊過程信息，最值得關注的條目是彈藥餘量和處理秒數（Processing Seconds）。

各炮組攜帶不同彈種，AI會在射擊時自動選擇最優彈藥。

處理秒數隨時間推進增加，直至達到射速（Rate of Fire）決定的閾值時觸發射擊判定，隨後歸零。未模擬飛行中的炮彈（但已發射的魚雷會持續模擬）。
...
グローバルログパネルにログエントリが表示されます。また、被弾艦の艦艇動態状態エディターを開き「基本」タブの「詳細」ボタンをクリックすると、個別艦艇レベルでの詳細ログを確認できます。
...
全局日志面板将出现日志条目。您也可通过打开受损舰艇的舰艇动态状态编辑器，点击"基本"标签页中的"详情"按钮，查看单舰层面的详细日志。
...
全局日志面板將出現日志條目。您也可通過打開受損艦艇的艦艇動態狀態編輯器，點擊「基本」標籤頁中的「詳情」按鈕，查看單艦層面的詳細日志。
...
影響を受けた艦艇はグローバルログで確認できるほか、艦艇動態状態エディターを開き「損傷効果」タブでキーボードの上下キーを使用して各艦の状況を確認できます。
...
您可通过全局日志判定受影响舰艇，或打开舰艇动态状态编辑器切换到"损伤效果"标签页，使用键盘上下键查看各舰状态。

每次命中会造成"均匀"的损伤点损失，而更"不均匀"且部位特定的损伤（如弹药库爆炸、进水、舵机失效、火控系统错位等）则由损伤效果处理。

部分损伤效果可能是永久性或临时性的，并显示在"损伤效果"标签页中。某些效果（特别是舰上火灾）是可损伤控制的。若未分配损伤控制点，这些效果可能持续恶化。AI将根据默认优先级使用其损伤控制点来抑制损伤效果。
`,
`
艦艇已施加損傷效果（子狀態）！

您可通過全局日志判定受影響艦艇，或打開艦艇動態狀態編輯器切換到「損傷效果」標籤頁，使用鍵盤上下鍵查看各艦狀態。
...
机制上，损伤点超过特定层级（百分比阈值）时可能触发致命"通用"损伤效果。若短期内过多层级被超越，舰艇将会沉没。但存在不可忽视的概率，舰艇可能未触发任何重大损伤效果就达到100%。超过此点后，额外损伤点不会增加沉没概率（尽管某些损伤效果仍可能导致沉没）。您可以将其理解为"已无物可爆"的状态——仅是漂浮海面的进水船体。在水上部分打出更多的窟窿并不会促进沉没。
`,
`
艦艇已被擊沉！
...
画面上部の「指揮」タブから「勝利状況」ダイアログを開くことができます。そこでは主力部隊の損失と損傷状況が報告されます。勝利ポイントは艦艇の損傷状態、火力、損傷点、装甲に基づいて計算されます。これらの値は艦艇型号エディター（静的値）と艦艇動態状態エディター（動的値）で確認できます。これらの値はAIによっても使用されます。艦艇を撃沈すると×2の修正が適用されます。
`,
`
编队已被摧毁！

您可以从顶部"指挥"标签页打开"胜利状态"对话框。该对话框将报告主力编队的损失和损伤情况。胜利点根据舰艇的损伤状态、火力、损伤点和装甲值计算。这些数值可在舰艇型号编辑器（静态值）和舰艇动态状态编辑器（动态值）中查看。AI同样使用这些数值计算。击沉舰艇会应用×2的修正系数。
`,
`
編隊已被摧毀！

您可以從頂部「指揮」標籤頁打開「勝利狀態」對話框。該對話框將報告主力編隊的損失和損傷情況。勝利點根據艦艇的損傷狀態、火力、損傷點和裝甲值計算。這些數值可在艦艇型號編輯器（靜態值）和艦艇動態狀態編輯器（動態值）中查看。AI同樣使用這些數值計算。擊沉艦艇會應用×2的修正係數。
`)
