...
射撃を行っているグループから艦艇を選択してください（本シナリオでは通常、日本艦隊が先に射撃します）。赤色の線が表示され、主砲・副砲・第3砲兵装・または速射砲による目標への射撃が可視化されます。

艦船状態ビューを開き「兵装」タブに移動してください。ここで射撃プロセスの情報を確認でき、特に弾薬残数と処理秒数が注目すべき項目です。

各砲兵装は異なる種類の弾薬を搭載しており、AIは射撃時に最適な弾種を自動選択します。

処理秒数は時間経過とともに増加し、射撃速度で決定された閾値に達すると、射撃が処理され処理秒数がゼロにリセットされます。

弾薬の飛翔時間はすでに射撃速度に含まれています（貫通力－射撃速度表を見ると、修正射撃では前回の着弾結果を観測してから次弾を発射する必要があるため、射撃速度が距離によって制限される場合と、砲自体のサイクル速度によって制限される場合があることが分かります）。弾丸の飛翔はモデル化されていませんが、弾が目標に到達した際（命中・未命中を問わず）、射撃側から目標に向けて、100倍に拡大された弾丸の飛行アニメーションが再生されます。これはあくまで視覚効果であり、実際の飛翔中の弾丸を表すものではありません。

各種砲架は、対応する艦級レコードで定義された固有の射界を持ちます。一般的に艦艇の舷側火力は前後方向の火力よりも強力ですが、舷側を露出させることは被弾しやすい姿勢でもあります。
...
部分损伤效果可能是永久性或临时性的，并显示在"损伤效果"标签页中。某些效果（特别是舰上火灾）可进行损害管制。若未分配损害管制点，这些效果可能持续恶化。AI将根据默认优先级使用其损害管制点来抑制损伤效果。
`,
`
艦艇已施加損傷效果（子狀態）！
...
部分損傷效果可能是永久性或臨時性的，並顯示在「損傷效果」標籤頁中。某些效果（特別是艦上火災）可進行損害管制。若未分配損害管制點，這些效果可能持續惡化。AI將根據默認優先級使用其損害管制點來抑制損傷效果。
`);
...
お気付きかもしれませんが、『日清戦争』（Seekrieg 5システム）では、損傷点が100%に達しても撃沈は保証されません。艦艇は100%に達する前に沈没することもあれば、1000%を超えても沈没しない場合もあります。損傷点は主に確率分布を形成するもので、艦艇は100%で戦闘不能になる傾向がありますが、完全な破壊は確定しないのです。

メカニズム的には、損傷点が特定のティア（パーセンテージしきい値）を超えると、致命的な「一般」損傷効果を引き起こす可能性があります。短期間に多数のティアを超えると艦艇は沈没します。しかし無視できない確率で、重大な損傷効果が発生せずに100%に達する場合もあります。この段階を超えると、追加の損傷点は沈没確率を上昇させません（ただし特定の損傷効果が沈没を引き起こす可能性は残ります）。これは「爆発するものがない状態」——海上を漂流する浸水した船体——と考えることができます。水上部分にさらに穴を開けても沈没には寄与しません。
`,
`
舰艇已被击沉！
