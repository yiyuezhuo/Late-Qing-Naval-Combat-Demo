...
次に松島の艦艇状態ビューを開き、砲兵装タブに切り替えます。副砲の項目までスクロールしてください。砲架は12基ありますが、発砲しているのは右舷側の6基だけです。

3つの砲架と1つのFCS位置を使って別の艦船を攻撃します。次の方法で行ってください：各砲架またはFCS位置について、設定ボタンをクリックし、その後別の艦船をクリックします。

攻撃制約とは異なり、この方法では目標が即座に切り替わります。

その後「ドクトリン」タブに切り替え、「自動射撃」の「継承」を無効にし、設定を「手動」にしてください。これでアルゴリズムが目標を再設定しなくなります。
...
然后打开松岛的舰船状态视图并切换到炮组标签。滚动到副炮区域。这里共有 12 个炮座，但只有右舷的 6 个炮座正在开火。

用其中三个炮座与一个火控位攻击另一个舰船——通过以下方式做到：对于每个炮座与火控位，点击设置按钮，然后点击另一个舰船。
...
然後打開松島的艦船狀態檢視並切換到炮組分頁。捲動到副炮區域。這裡共有 12 個砲座，但只有右舷的 6 個砲座正在開火。

使用其中三個砲座與一個FCS位攻擊另一艘艦船——方法如下：對於每個砲座或FCS位，點擊設定按鈕，然後點擊另一艘艦船。
...
`,
`
ご覧のとおり、対応する「自動化」ドクトリンが無効化されているため、アルゴリズムはもう目標を変更しません。
