...
If edit mode is not enabled, switch to the Command tab in the top tab bar and enable Edit Mode first.
...
編集モードが有効になっていない場合は、上部タブバーの「コマンド」タブに切り替え、先に「編集モード」を有効にしてください。

挿入（Ins）キー、または上部タブバーの「エディタ」タブにある対応ボタンを押してから、マップ上の任意の場所をクリックしてユニットを挿入してください。
`,
`
...
如果尚未开启编辑模式，请先切换到顶部标签栏的“命令”标签页，并启用“编辑模式”。
...
如果尚未啟用編輯模式，請先切換到頂部標籤列的「命令」標籤頁，並啟用「編輯模式」。
